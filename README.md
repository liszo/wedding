...
| Path | What |
|---|---|
| `content/config.ts` | **Edit this first.** Names, dates, venue (including the Neshan place link), running order. Everything user-facing reads from it. `rsvpDeadlineISO` is `null`, which means the form never closes; `rsvpDeadlineFa` is still shown, because asking for a reply by a date and refusing one after it are different things. Put an ISO string back and both the form and `/api/rsvp` start enforcing it — they read the same `rsvpClosed()`. |
